...
import { RenderVnode, typeIconMap } from "@liu-element/utils";
...
<template>
    <transition :name="`liu-notification-${transitionName}`" @after-leave="!visible && onDestory()"
        @enter="boxHeight = notifyRef!.getBoundingClientRect().height">
        <div ref="notifyRef" class="liu-notification" :class="{
            [`liu-notification--${type}`]: type,
            'show-close': showClose,
            [horizontalClass]: true,
        }" :style="cssStyle" v-show="visible" role="alert" @click="onClick" @mouseenter="clearTimer"
            @mouseleave="startTimer">
            <liu-icon v-if="iconName" :icon="iconName" class="liu-notification__icon" />

            <div class="liu-notification__text">
                <div class="liu-notification__title">{{ title }}</div>
                <div class="liu-notification__content">
                    <slot>
                        <render-vnode v-if="message" :vNode="message" />
                    </slot>
                </div>
            </div>
            <div class="liu-notification__close" v-if="showClose">
