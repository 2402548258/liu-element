...
import { useZIndex } from "@liu-element/hooks";
import LoadingComp from "./Loading.vue";
import {  delay, isNil, isString } from "lodash-es";


const RELATIVE_CLASS = "liu-loading-parent--relative" as const;
const HIDDEN_CLASS = "liu-loading-parent--hidden" as const;
const LOADING_NUMB_KEY = "liu-loading-numb" as const;
...
function createLoadingComponent(options: LoadingOptionsResolved) {
  ...
  const destroy = () => {
    const target = data.parent as HTMLElement 
...
export function Loading(options: LoadingOptions = {}): LoadingInstance {
  ...
  const target = resolved.target ?? document.body;
